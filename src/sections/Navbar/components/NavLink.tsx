...
export const NavLink = (props: NavLinkProps) => {
  ...
  return (
    <a
      ...
      title={props.title}
      onClick={handleClick}
      className={`group items-center box-border caret-transparent gap-x-2 inline-flex gap-y-2 px-3 md:px-4 py-2 hover:bg-blue-600/10 hover:text-blue-500 rounded-lg transition-colors ${props.isActive ? "bg-neutral-900/50" : ""
        }`}
    >
      {props.iconSrc && (
        <img
      ...
      <span className="hidden md:block text-sm font-medium box-border caret-transparent leading-5">
        {props.label}
      </span>
    </a>
  );
